"""
GotfriendsScraper — scrapes job listings from gotfriends.co.il.

GotFriends is an Israeli staffing and recruitment agency.  Their career site
lists open positions on behalf of client companies.  Because many clients wish
to remain anonymous the "company" field is frequently absent or contains a
generic placeholder such as "חברה בתחום הטכנולוגיה" — these are normalised to
"GotFriends (Confidential)".
...
The synchronous scrape_gotfriends_jd() helper is kept for url_router / backfill.
"""
...
_AGENCY_LABEL = "GotFriends (Confidential)"
...
# ── Sync fetch helper (used by scrape_gotfriends_jd / url_router) ─────────────
...
def _fetch_html(url: str, session: requests.Session) -> Optional[BeautifulSoup]:
    try:
        ...
        logger.warning("[GotfriendsScraper] Failed to fetch %s: %s", url, exc)
        return None
...
def _parse_job_cards(soup: BeautifulSoup) -> list[dict]:
    """Extract job card data from a listing page."""
    items = (
        soup.find_all("article", class_=re.compile(r"job", re.I))
        or soup.find_all("div", class_=re.compile(r"job[\-_]?(item|card|listing|row)", re.I))
    ...
    for item in items:
        ...
        title_el = (
            item.find(["h2", "h3", "h4"], class_=re.compile(r"title|position|role", re.I))
        ...
        location_el = item.find(class_=re.compile(r"location|city|area", re.I))
...
def _parse_detail_page(soup: BeautifulSoup) -> str:
    """Extract JD text from a job detail page."""
...
def scrape_gotfriends_jd(url: str) -> str:
    """
    Fetch the full JD text for a single GotFriends job URL.
    Used by url_router for backfill and inline card fetch (sync).
    """
    session = requests.Session()
    soup = _fetch_html(url, session)
    if soup is None:
        raise ValueError(f"[GotfriendsScraper] Could not load {url}")
    text = _parse_detail_page(soup)
    if len(text) < 50:
        raise ValueError(f"[GotfriendsScraper] JD text too short ({len(text)} chars) for {url}")
    return text
...
class GotfriendsScraper(BaseScraper):
    """
    Scrapes open positions from gotfriends.co.il.
    ...
    user_id  : owner of the resulting JobMatch records
    max_jobs : safety cap on the total number of detail pages fetched per run
    """

    def __init__(
        self,
        company_name: str = "GotFriends",
    ...
    async def _gather_listing_cards(self, client: httpx.AsyncClient) -> list[dict]:
        ...
        logger.info(
            "[GotfriendsScraper] Found %d unique cards across %d keyword search(es)",
    ...
    async def _fetch_detail(
        ...
        return minimal_job_match(
            job_id      = make_job_id(card["url"], prefix="gotfriends"),
            title       = card["title"],
            company     = company,
    ...
    async def fetch_jobs(self) -> list[JobMatch]:
        ...
        async with httpx.AsyncClient(
            ...
            if discarded:
                logger.info(
                    "[GotfriendsScraper] Discarded %d irrelevant title(s) before detail fetch",
                    discarded,
                )
        ...
        logger.info("[GotfriendsScraper] Returning %d job(s)", len(results))
        return results
